...
tokens with dark fallbacks, so print tokens still apply.
"""
...
LIGHT_FILL = {
    "night": ("var(--bg, #141810)", 0.85),
    "dusk/dawn": ("var(--water, #8fa7b8)", 0.20),
    "sunset/sunrise": ("var(--water, #8fa7b8)", 0.16),
    "golden": ("var(--gold, #d3aa5f)", 0.16),
    "moderate day": ("var(--ink, #e8e3d2)", 0.06),
    "bright day": ("var(--ink, #e8e3d2)", 0.09),
}
...
def session_timeline_svg(m: dict) -> str:
    ...
    for b in blocks:
        ...
        fill, op = LIGHT_FILL.get(b.get("light", ""), ("var(--ink, #e8e3d2)", 0.05))
        ...
        parts.append(f'<line x1="{bx0:.1f}" y1="{ytop}" x2="{bx0:.1f}" y2="{ybot}" '
                     f'stroke="var(--line2, #2a321f)" stroke-width="1"/>')
    parts.append(f'<line x1="{x0}" y1="{ybot}" x2="{x1}" y2="{ybot}" '
                 f'stroke="var(--line, #37402c)" stroke-width="1"/>')
    ...
    if len(pts) >= 2:
        parts.append(f'<polyline points="{" ".join(pts)}" fill="none" '
                     f'stroke="var(--water, #8fa7b8)" stroke-width="2" opacity=".9"/>')
    ...
    if len(presses) >= 2:
        ...
        if len(ppts) >= 2:
            parts.append(f'<polyline points="{" ".join(ppts)}" fill="none" '
                         f'stroke="var(--gold, #d3aa5f)" stroke-width="2"/>')
            parts.append(_text(4, ytop + 8, f"{hi:.0f}", fill="var(--gold, #d3aa5f)",
                               font_size="10", font_family=mono))
            parts.append(_text(4, ybot - 2, f"{lo:.0f}", fill="var(--gold, #d3aa5f)",
                               font_size="10", font_family=mono))
    ...
    for i, (dt, lab) in enumerate(sorted(marks)[:6]):
        ...
        parts.append(f'<line x1="{ex:.1f}" y1="{ytop}" x2="{ex:.1f}" y2="{ybot}" '
                     f'stroke="var(--dim, #a09c85)" stroke-width="1" '
                     f'stroke-dasharray="3 3" opacity=".8"/>')
        parts.append(_text(ex, 12 + (i % 2) * 9, lab, fill="var(--dim, #a09c85)",
    ...
    if prime:
        ...
        parts.append(f'<rect x="{px0:.1f}" y="{ytop}" width="{max(2.0, px1 - px0):.1f}" '
                     f'height="{ybot - ytop}" fill="none" stroke="var(--gold, #d3aa5f)" '
        ...
        if picks:
            label += f" · {picks}"
        parts.append(_text(px0, 164, label, fill="var(--gold, #d3aa5f)",
    ...
    while tick <= t1:
        ...
        parts.append(f'<line x1="{tx:.1f}" y1="{ybot}" x2="{tx:.1f}" y2="{ybot + 4}" '
                     f'stroke="var(--line, #37402c)" stroke-width="1"/>')
        parts.append(_text(tx, ybot + 15, tick.strftime("%-I %p"), fill="var(--dim, #a09c85)",
                           font_size="9.5", text_anchor="middle", font_family=mono))
        tick += timedelta(seconds=step)

    parts.append(_text(x1, 184, "sun altitude · barometer", fill="var(--dim, #a09c85)",
